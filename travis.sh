...
dpkg --add-architecture i386 && apt-get update && apt-get install -y git ccache automake bc lzop bison gperf build-essential zip clang-7 lldb-7 lld-7 curl zlib1g-dev zlib1g-dev:i386 g++-multilib python-networkx libxml2-utils bzip2 libbz2-dev libbz2-1.0 libghc-bzlib-dev squashfs-tools pngcrush schedtool dpkg-dev liblz4-tool make optipng &&
...
git clone https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86/+/master/clang-4053586/ /pipeline/build/root/toolchain/aarch64-linux-android-clang
KERNEL_DIR=$PWD
...
curl -s -X POST https://api.telegram.org/bot$BOT_API_KEY/sendMessage -d text="New kernel build started!" -d chat_id=@andreabuilds;
